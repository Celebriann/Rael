...
# -*- coding: utf-8 -*-
"""
UI.py — Interface TUI pour le client IRC
Basé sur Textual + Rich

Installation : pip install textual

Utilisation : importer ChatApp depuis client.py
"""

from textual.app import App, ComposeResult
from textual.widgets import Input, RichLog, Label, Button, Header
...
AMBER       = "#FFB300"   # Texte principal
AMBER_DIM   = "#7A5500"   # Texte secondaire / séparateurs
AMBER_BRIGHT= "#FFD54F"   # Accents forts
GREEN       = "#69FF47"   # Pseudos
CYAN        = "#00E5FF"   # Noms de channels
RED         = "#FF3D3D"   # Erreurs / avertissements
BG          = "#0A0A0A"   # Fond
BG_INPUT    = "#111111"   # Fond input
BG_PANEL    = "#0F0F0F"   # Fond panneaux
...
def format_message(raw: str) -> Text:
    """Transforme une ligne brute en Text Rich coloré."""
    t = Text()
    ...
    if raw.startswith("[MP"):
        match = re.match(r'\[MP (de|a) (.+?)\] (.*)', raw)
        if match:
            direction, who, msg = match.groups()
    ...
    # Message de channel : [#channel] pseudo: texte
    match = re.match(r'\[#(.+?)\] (.+?): (.*)', raw)
    if match:
        channel, nick, msg = match.groups()
        t.append("[", style=AMBER_DIM)
...
def format_channel(name: str) -> Text:
    t = Text()
    t.append(f"#{name}", style=Style(color=CYAN, bold=True))
    return t
...
class AuthScreen(Screen):
    ...
    def __init__(self, on_login, on_create):
        ...
        self._mode = "login"   # "login" ou "create"

    def compose(self) -> ComposeResult:
        with Container(id="auth_screen"):
            with Vertical(id="auth_box"):
                yield Label("◈  RAEL  ◈ Version: 1.1", id="auth_title")
                yield Label("Tor · TLS 1.3 · E2E RSA+AES", id="auth_subtitle")

                yield Label("Pseudo", classes="auth_label")
                yield Input(placeholder="votre_pseudo", id="nick_input",
                           classes="auth_input")

                yield Label("Mot de passe", classes="auth_label")
                yield Input(placeholder="••••••••", password=True,
                           id="pass_input", classes="auth_input")

                yield Label("", id="extra_label", classes="auth_label")
                yield Input(placeholder="", id="extra_input",
                           classes="auth_input")

                with Horizontal(id="auth_buttons"):
                    yield Button("Connexion", id="btn_login",
                                classes="btn_primary")
                    yield Button("Créer un compte", id="btn_switch",
...
class ChatScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="chat_layout"):
            ...
            with Horizontal(id="input_bar"):
                ...
                yield Input(placeholder="Tapez un message ou /commande...",
                           id="msg_input")
    ...
    def push_line(self, raw: str):
        """Ajoute une ligne formatée dans le log."""
        log = self.query_one("#messages", RichLog)
        log.write(format_message(raw))
    ...
    def set_channel(self, channel: str | None):
        ...
        if channel:
            ...
            self.query_one("#prompt", Label).update(
                Text.assemble(
                    ("#" + channel, Style(color=CYAN, bold=True)),
    ...
    def set_tor_status(self, active: bool):
        self.query_one("#status_tor", Label).update(
            "● TOR" if active else "○ LOCAL"
        )
...
class ChatApp(App):
    ...
    def on_mount(self):
        if self._pending_auth:
            self.push_screen(self._auth_screen)

    def start_auth(self, on_auth_done):
        """
        Affiche l'écran d'authentification.
        on_auth_done(nick, pwd, bio) appelé quand le formulaire est soumis.
        bio=None si mode login.
        """
        self._auth_cb = on_auth_done
        self._auth_screen = AuthScreen(
            on_login=lambda n, p: on_auth_done(n, p, None),
            on_create=lambda n, p, b: on_auth_done(n, p, b)
        )
        self._pending_auth = True
        # Si deja monte, pousser directement
        try:
            self.push_screen(self._auth_screen)
        except Exception:
            pass
    ...
    def start_chat(self, nick: str, on_send, tor_active=True):
        """
        Passe à l'écran de chat en plein écran.
        on_send(message) appelé à chaque message envoyé.
        """
        self._send_cb     = on_send
        self._chat_screen = ChatScreen(
            on_send=on_send,
            nick=nick,
            tor_active=tor_active
        )
        if self._auth_screen:
            self.pop_screen()
    ...
    # ── API publique pour client.py ──

    def auth_error(self, msg: str):
        """Affiche une erreur sur l'écran d'auth."""
        if self._auth_screen:
            self._auth_screen.show_error(msg)
...
if __name__ == "__main__":
    """Lance l'UI en mode démo pour tester sans le serveur."""

    import asyncio

    class DemoApp(ChatApp):
        def on_mount(self):
            def fake_auth(nick, pwd, bio):
                self.start_chat(nick, self.fake_send, tor_active=True)
                self.push_message(f"[*] Connecté en tant que {nick}")
                self.push_message("[*] Ceci est un mode démo sans serveur")
                self.push_message("[*] Rejoint le salon #général automatiquement")
                self.set_channel("général")
                self.push_message("[#général] Système: Bienvenue dans le démo !")
                self.push_message("[#général] Alice: Salut tout le monde 👋")
                self.push_message("[#général] Bob: Hey ! Comment ça va ?")
                self.push_message("[!] Ceci est une erreur exemple")
                self.push_message("--- Séparateur exemple ---")

            self.start_auth(fake_auth)

        def fake_send(self, msg):
            nick = self._chat_screen._nick if self._chat_screen else "moi"
            self.push_message(f"[#{self._chat_screen._channel or 'général'}] {nick}: {msg}")
