...
# ─── Historique persistant sur disque ────────────────────────────────────────

def history_file(channel):
    os.makedirs(HISTORY_DIR, exist_ok=True)
    return os.path.join(HISTORY_DIR, f"{channel}.json")

def save_history(channel, history, users):
    try:
        with open(history_file(channel), "w", encoding="utf-8") as f:
            json.dump({"history": history, "users": list(users)}, f, ensure_ascii=False)
    except Exception as e:
        print(f"[!] Erreur sauvegarde historique #{channel}: {e}")

def load_history(channel):
    fn = history_file(channel)
    if not os.path.exists(fn):
        return [], set()
    try:
        with open(fn, "r", encoding="utf-8") as f:
            data = json.load(f)
        return data.get("history", []), set(data.get("users", []))
    except:
        return [], set()
...
def send_history(sock, channel):
    """Envoie l'historique stocké en RAM au membre qui rejoint."""
    with lock:
    ...
    for line in history:
        send(sock, line)
    send(sock, f"--- Fin de l'historique ---")
...
def leave_channel(sock):
    ...
    if is_owner:
        ...
        save_history(channel, history, users)
        print(f"[*] Salon #{channel} hors ligne — historique sauvegardé ({len(history)} msgs)")
    else:
        with lock:
...
def handle_message(sock, message):
    ...
    if message.startswith("__HISTORY_RESET__"):
        # Le créateur renvoie son historique - vider la RAM d'abord
        salon_name = message[17:]
        with lock:
            if salon_name in channels:
                channels[salon_name]['history'] = []
        return
        ...
        try:
            ...
            with lock:
                ...
                if a_nick in registered_nicks:
                    # Comparaison directe — le client envoie toujours pbkdf2 deterministe
    ...
    if message.startswith("/"):
        ...
        if command == "/help":
            ...
            # Après redémarrage serveur : offline_salons est vide mais le fichier JSON existe
            if not already_exists and not is_offline:
                saved_h, saved_u = load_history(new_channel)
                if saved_u and nick in saved_u:
                    is_offline = True
                    is_returning_owner = True
                    with lock:
                        offline_salons[new_channel] = nick

            if already_exists:
                with lock:
                    # Verifier si c'est un utilisateur connu du salon AVANT de l'ajouter
                    is_returning_member = nick in channels[new_channel].get('users', set())
                ...
                # Historique uniquement pour les membres qui ont deja ete dans ce salon
                if is_returning_member:
                    send_history(sock, new_channel)

            elif is_returning_owner:
                # Charger l'historique depuis le disque
                saved_history, saved_users = load_history(new_channel)
                with lock:
                    saved_users_lock = salon_keys.pop(new_channel + '__users', set())
                    saved_users = saved_users | saved_users_lock
                ...
                # Envoyer l'historique au créateur
                send_history(sock, new_channel)
        ...
        with lock:
            if channel in channels:
                ...
                history_snap = channels[channel]['history'].copy()
                users_snap   = channels[channel]['users'].copy()
        broadcast_channel(channel, formatted, exclude=sock)
        send(sock, formatted)
        # Sauvegarder sur disque à chaque message (survie au redémarrage serveur)
        save_history(channel, history_snap, users_snap)
        with lock:
            owner_sock = channels.get(channel, {}).get('owner')
        if owner_sock and owner_sock != sock:
